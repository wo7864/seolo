...
saver_path = '/content/gdrive/My Drive/Korean_model/model/'
make_image_path = '/content/gdrive/My Drive/Korean_model/image/'
	...
	for r in range(10):	
		...
		for m in range(10):
			...
			for k in range(0, space):
				#category 변환
				latent_code = np.zeros([num_generate, 3])
				...
				max_v = 0
				for idx, i in enumerate(generated[0]):
					for idx2, j in enumerate(i):
						if j < 0:
							generated[0][idx][idx2] = 0
						if max_v < j:
							max_v = j
		...
		for i in range(space):
			for j in range(space):
				plt.subplot(space,space,i*10+j+1)
				plt.imshow(image[i][j][0], cmap='Greys_r')
				plt.axis('off')

		plt.show()
		plt.close()	
phoneme_list = [x for x in range(1, 10)]
for i in phoneme_list:
	...
	model.saver.restore(sess, "gdrive/My Drive/Korean_model/model/type6/type6_{}.ckpt".format(str(i)))
