...
def img_attach(font, imgs, text, latter_attach=60, definition=30, color=0, phoneme_width_attach=30, phoneme_height_attach=30):
    save_dir = "./result/"
    latter_size = 84
    img_size1 = 28
    img_size2 = 42
    ...
    filename = "{}_{}_{}_{}.png".format(font, latter_attach, definition, color)
...
def main(font_list, model_list, z_list, font, text, latter_attach, definition, color):
    text = convert_text(text)

    generated_images = random_generate(font, model_list, z_list, text)
    result = img_attach(font, generated_images, text, latter_attach, definition, color)
    plt.show()
    plt.close(result)
