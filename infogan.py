...
class GAN:
    ...
    def __init__(self, sess):
        ...
        with tf.name_scope("generate_image_from_noise"):
            self.Gen = self.Generator(self.noise_source,
                                      self.latent_code)  # batch_size, self.height, self.width, self.channel
        ...
        with tf.name_scope("train"):
            ...
            with tf.control_dependencies(tf.get_collection(tf.GraphKeys.UPDATE_OPS)):
                ...
                self.D_minimize = tf.train.AdamOptimizer(
                    learning_rate=self.train_rate_D_Q, beta1=0.5).minimize(self.D_loss,
                                                                           var_list=self.D_variables)  # D 변수만 학습.
                self.G_minimize = tf.train.AdamOptimizer(
                    learning_rate=self.train_rate_G, beta1=0.5).minimize(self.G_loss,
                                                                         var_list=self.G_variables)  # G 변수만 학습.
                self.Q_minimize = tf.train.AdamOptimizer(
                    learning_rate=self.train_rate_D_Q, beta1=0.5).minimize(self.Q_loss, var_list=[self.G_variables,
                                                                                                  self.Q_variables])  # G, Q 변수만 학습.
    ...
    def Generate_noise(self, batch_size):  # batch_size, 1, 1, self.noise_dim
        return np.random.uniform(-1.0, 1.0,
                                 size=[batch_size, self.noise_dim])  # -1~1의 uniform distribution DCGAN논문에서 나온대로.

    # latent code c 생성
    def Generate_latent_code(self, batch_size):
        # https://docs.scipy.org/doc/numpy/reference/generated/numpy.random.multinomial.html
        discrete_c = np.random.multinomial(1, [1 / self.categorical] * self.categorical,
                                           size=batch_size)  # batch, self.categorical
    ...
    def Discriminator(self, data, reuse=False):  # batch_size, 1, 1, 1
        with tf.variable_scope('Discriminator') as scope:
            ...
            # input layer는 BN 안함.
            D_conv1 = tf.layers.conv2d(inputs=data, filters=64, kernel_size=[4, 4], strides=(2, 2),
                                       padding='same')  # batch, 14, 14, 64
            D_conv1 = tf.nn.leaky_relu(D_conv1)  # default leak is 0.2

            D_conv2 = tf.layers.conv2d(inputs=D_conv1, filters=128, kernel_size=[4, 4], strides=(2, 2),
                                       padding='same')  # batch, 7, 7, 128
        ...
        if reuse == True:
            with tf.variable_scope('Q'):
                ...
                D_fc3_Q = tf.layers.dense(D_fc2_Q, self.categorical + self.continuous,
                                          activation=None)  # batch, self.categorical+self.continuous
    ...
    def Generator(self, noise, latent_code):  # batch_size, self.height, self.width, self.channel
        ...
        with tf.variable_scope('Generator'):
            ...
            G_upconv2 = tf.layers.conv2d_transpose(inputs=G_upconv1, filters=self.channel, kernel_size=[4, 4],
                                                   strides=(2, 2), padding='same')  # batch, 28, 28, 1
