...
class Phoneme:
    def __init__(self, img, shape, latter_num, phoneme_num, x_point, y_point):
...
def create_latter_list(imgs, text, shape_list):
    ...
    for idx, latter in enumerate(text):
        ...
        for idx2, pho in enumerate(latter):
            if text[idx][idx2] != 26:
                phoneme_list2.append(Phoneme(imgs[idx][idx2], shape_list[idx], idx, idx2, x_point, y_point))
...
def img_attach(imgs, latter_list, latter_attach, definition, color, filename):
    save_dir = "./result/"
    max_latter_size = 84
    if latter_attach > max_latter_size:
        width = latter_attach * (len(imgs)-1)
    else:
        width = latter_attach * (len(imgs)-1) + (max_latter_size - latter_attach)
    result = np.zeros((max_latter_size, width * 2))
    result.fill(1)
...
def create_one_image(font, model_list, z_list, text):
    img = model_list[font][text].predict(z_list[font][text])[79]
    return img[0]
...
def main(font_list, model_list, z_list, font, ori_text, latter_attach, definition, color, filename):
    ...
    latter_list = create_latter_list(generated_images, text, shape_list)
    result, latter_list = img_attach(generated_images, latter_list, latter_attach, definition, color, filename)
    target = latter_list[0][1]
    tmp = np.array(generated_images)
    img = create_one_image(font, model_list, z_list, 7)
